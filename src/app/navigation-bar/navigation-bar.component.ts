...
export class NavigationBarComponent implements OnInit {
  loggedInUserData$: Observable<LoggedInUser> = new Observable<LoggedInUser>();
  constructor(private store$: Store<fromApplicationStore.GlobalStore>) {}

  ngOnInit(): void {
    this.loggedInUserData$ = this.store$.select(
      fromAuthenticationSelectors.getLoggedInUser
    );
  }
